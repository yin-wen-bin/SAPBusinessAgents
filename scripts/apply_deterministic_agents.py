...
def sap_step(
    ...
    *,
    ...
    for key in ["steps", "select_fields", "filters", "filter_from_previous"]:
...
def execution(
    ...
    *,
    ...
    evidence = {step["id"]: f"{{{{steps.{step['id']}.output}}}}" for step in sap_steps}
    return {
        "mode": "deterministic",
        "timeoutSeconds": 300,
...
def specs() -> dict[str, dict[str, Any]]:
    ...
    fi_fields = ["CompanyCode", "FiscalYear", "FiscalPeriod", "AccountingDocument", "AccountingDocumentItem", "Supplier", "Customer", "GLAccount", "OperationalGLAccount", "PurchasingDocument", "PurchasingDocumentItem", "PostingDate", "NetDueDate", "IsCleared", "ClearingAccountingDocument", "ClearingDocFiscalYear", "PaymentMethod", "PaymentBlockingReason", "HouseBank", "HouseBankAccount", "AccountingDocumentType", "AmountInTransactionCurrency", "TransactionCurrency", "OrderID"]
    ...
    ap_plan = [query("supplier_items", "API_OPLACCTGDOCITEMCUBE_SRV", "A_OperationalAcctgDocItemCube", fields=fi_fields, filters=[filt("CompanyCode", "{{input.company_code}}"), filt("Supplier", "{{input.supplier}}")], rationale="Load supplier open, cleared, due, and payment evidence.")]
    result["ap-payment"] = execution("ap-payment", ap_inputs, [sap_step("collect_ap_evidence", ap_plan, rationale="Collect supplier payment evidence.")], known_gaps=["payment_run_and_bank_master_evidence"])
    ...
    ar_plan = [query("customer_items", "API_OPLACCTGDOCITEMCUBE_SRV", "A_OperationalAcctgDocItemCube", fields=fi_fields, filters=[filt("CompanyCode", "{{input.company_code}}"), filt("Customer", "{{input.customer}}")], rationale="Load customer open, cleared, due, and collection evidence.")]
    result["ar-collection"] = execution("ar-collection", ar_inputs, [sap_step("collect_ar_evidence", ar_plan, rationale="Collect customer collection evidence.")], known_gaps=["bank_receipt_matching_evidence"])
    ...
    grir_plan = [
        query("gl_items", "API_GLACCOUNTLINEITEM", "GLAccountLineItem", fields=["CompanyCode", "FiscalYear", "AccountingDocument", "AccountingDocumentItem", "GLAccount", "PurchasingDocument", "PurchasingDocumentItem", "PostingDate", "AmountInCompanyCodeCurrency", "CompanyCodeCurrency", "DebitCreditCode"], filters=[filt("CompanyCode", "{{input.company_code}}"), filt("GLAccount", "{{input.gl_account}}"), filt("PostingDate", "{{input.date_from}}", op="ge", kind="date_start"), filt("PostingDate", "{{input.date_to}}", op="le", kind="date_end")], rationale="Load scoped GR/IR G/L line items."),
        query("purchase_order_items", "API_PURCHASEORDER_PROCESS_SRV", "A_PurchaseOrderItem", fields=["PurchaseOrder", "PurchaseOrderItem", "Material", "Plant", "OrderQuantity", "OrderPriceUnit", "DocumentCurrency"], bindings=[bind("PurchaseOrder", "gl_items", "PurchasingDocument")], rationale="Load purchase-order items referenced by GR/IR lines."),
    ...
    close_inputs = schema({
        ...
        "sap_client": string_input("SAP Client", "SAP client", placeholder_zh="例如：100", placeholder_en="Example: 100", pattern=DIGITS, maximum=3),
    })
    close_plan = [query("fi_period_items", "API_OPLACCTGDOCITEMCUBE_SRV", "A_OperationalAcctgDocItemCube", fields=fi_fields, filters=[filt("CompanyCode", "{{input.company_code}}"), filt("FiscalYear", "{{input.fiscal_year}}"), filt("FiscalPeriod", "{{input.period}}")], rationale="Load scoped FI period evidence without performing closing actions.")]
    ...
    result["billing-block-diagnosis"] = execution("billing-block-diagnosis", sales_order_input, [sap_step("collect_billing_block_evidence", exact_sales_order_plan(), rationale="Collect sales-order and delivery block evidence.")])
    ...
    result["billing-dispute-classification"] = execution("billing-dispute-classification", billing_input, [sap_step("collect_dispute_base_evidence", dispute_plan, rationale="Collect billing and FI evidence.")], known_gaps=["billing_dispute_text_evidence"])
    ...
    demand_plan = [
        ...
        query("sales_order_items", "API_SALES_ORDER_SRV", "A_SalesOrderItem", fields=["SalesOrder", "SalesOrderItem", "Material", "ProductionPlant", "RequestedQuantity", "ConfdDelivQtyInOrderQtyUnit", "DeliveryPriority", "DeliveryStatus"], bindings=[bind("SalesOrder", "sales_orders", "SalesOrder")], rationale="Load items for selected sales orders."),
        ...
        query("delivery_headers", "API_OUTBOUND_DELIVERY_SRV", "A_OutbDeliveryHeader", fields=["DeliveryDocument", "SalesOrganization", "PlannedGoodsIssueDate", "ActualGoodsMovementDate", "OverallGoodsMovementStatus", "DeliveryBlockReason", "TotalCreditCheckStatus"], filters=[filt("SalesOrganization", "{{input.sales_organization}}"), filt("PlannedGoodsIssueDate", "{{input.date_from}}", op="ge", kind="date_start"), filt("PlannedGoodsIssueDate", "{{input.date_to}}", op="le", kind="date_end")], rationale="Load delivery execution evidence for the same organizational period."),
    ]
    ...
    result["order-to-cash-anomaly-monitor"] = execution("order-to-cash-anomaly-monitor", organization_range, [sap_step("collect_o2c_anomaly_evidence", anomaly_plan, rationale="Collect bounded O2C anomaly evidence.")], known_gaps=["billing_output_and_dispute_evidence"])
    ...
    return_plan = [
        ...
        query("billing_documents", "API_BILLING_DOCUMENT_SRV", "A_BillingDocumentItem", fields=["BillingDocument", "BillingDocumentItem", "ReferenceSDDocument", "ReferenceSDDocumentItem", "BillingQuantity", "NetAmount", "TransactionCurrency"], bindings=[bind("ReferenceSDDocument", "credit_requests", "CreditMemoRequest")], rationale="Load follow-on billing for credit requests."),
    ]
    ...
    forecast_plan = [
        query("sales_demand", "API_SALES_ORDER_SRV", "A_SalesOrderItem", fields=["SalesOrder", "SalesOrderItem", "Material", "ProductionPlant", "RequestedQuantity", "ConfdDelivQtyInOrderQtyUnit"], filters=[filt("ProductionPlant", "{{input.plant}}"), filt("Material", "{{input.material}}")], rationale="Load historical sales demand."),
    ...
    supply = [query("supply_demand_items", "API_MRP_MATERIALS_SRV_01", "SupplyDemandItems", fields=["Material", "MRPArea", "MRPPlant", "MaterialShortageProfile", "MaterialShortageProfileCount", "MRPElement", "MRPElementCategory", "MRPElementAvailyOrRqmtDate", "MRPElementOpenQuantity", "MRPAvailableQuantity", "ExceptionMessageNumber", "ExceptionMessageText"], filters=[filt("Material", "{{input.material}}"), filt("MRPArea", "{{input.mrp_area}}"), filt("MRPPlant", "{{input.plant}}"), filt("MaterialShortageProfile", "{{input.shortage_profile}}"), filt("MaterialShortageProfileCount", "{{input.shortage_counter}}")], rationale="Load MRP supply-demand items.")]
    ...
    prod_plan = [
        ...
        query("production_operations", "API_PRODUCTION_ORDER_2_SRV", "A_ProductionOrderOperation_2", fields=["ManufacturingOrder", "ManufacturingOrderOperation", "WorkCenter", "OperationIsConfirmed", "OperationIsPartiallyConfirmed", "OpPlannedTotalQuantity", "OpTotalConfirmedYieldQty"], filters=[filt("ManufacturingOrder", "{{input.manufacturing_order}}")], rationale="Load production operations and confirmations."),
    ...
    schedule_plan = [
        query("planned_orders", "API_PLANNED_ORDERS", "A_PlannedOrder", fields=["PlannedOrder", "Material", "ProductionPlant", "TotalQuantity", "PlndOrderPlannedStartDate", "PlndOrderPlannedEndDate", "PlannedOrderIsFirm", "PlannedOrderIsConvertible", "CapacityRequirement"], filters=[filt("ProductionPlant", "{{input.plant}}"), filt("PlndOrderPlannedStartDate", "{{input.date_from}}", op="ge", kind="date_start"), filt("PlndOrderPlannedStartDate", "{{input.date_to}}", op="le", kind="date_end")], rationale="Load planned orders in the scheduling horizon."),
        query("planned_capacities", "API_PLANNED_ORDERS", "A_PlannedOrderCapacity", fields=["CapacityRequirement", "CapacityRequirementItem", "CapacityRqmtItemCapacity", "PlannedOrder", "WorkCenter", "CapacityCategoryCode", "Capacity", "OperationEarliestStartDate", "OperationEarliestEndDate", "CapacityRequirementUnit", "ScheduledCapReqOpSegProcgDurn", "RemainingCapReqOpSegProcgDurn"], bindings=[bind("PlannedOrder", "planned_orders", "PlannedOrder")], rationale="Load planned-order capacity requirements."),
        query("production_operations", "API_PRODUCTION_ORDER_2_SRV", "A_ProductionOrderOperation_2", fields=["ManufacturingOrder", "ManufacturingOrderOperation", "ProductionPlant", "WorkCenter", "OperationIsConfirmed", "OpPlannedTotalQuantity", "OpTotalConfirmedYieldQty", "OpActualExecutionStartDate", "OpActualExecutionEndDate"], filters=[filt("ProductionPlant", "{{input.plant}}"), filt("WorkCenter", "{{input.work_center}}")], rationale="Load production operations for the selected work center."),
...
def update_relationship_catalog() -> None:
    ...
    semantic_keys = {(item["service_name"], item["entity_set"], item["field"]) for item in semantics}
    ...
    for relation_id, source, target in RELATIONSHIPS:
        for service, entity, field, semantic in (source, target):
            ...
            if key not in semantic_keys:
                semantics.append({"service_name": service, "entity_set": entity, "field": field, "semantic": semantic})
                semantic_keys.add(key)
        if relation_id not in relationship_ids:
            relationships.append({
                "id": relation_id,
                "modes": ["binding"],
                "source": {"service_name": source[0], "entity_set": source[1], "field": source[2]},
                "target": {"service_name": target[0], "entity_set": target[1], "field": target[2]},
            })
