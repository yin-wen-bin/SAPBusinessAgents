...
test("fixed Agent lifecycle management is available in both languages", async () => {
  ...
  const draftStyles = await readFile(path.join("src", "styles", "agent-draft.css"), "utf8");
...
test("dual-mode prototype renders free-query and run pages", async () => {
  ...
  const pluginsEn = await readPage("en", "plugins");
  ...
  assert.doesNotMatch(run, /\.innerHTML\s*=/);
  assert.match(plugins, /<h1>插件<\/h1>/);
  assert.match(plugins, /data-plugin-manager/);
  assert.match(plugins, /data-plugin-overview/);
  assert.match(plugins, /data-mail-connections/);
  assert.match(plugins, /企业微信/);
  assert.match(plugins, /规划中/);
  assert.match(plugins, /不能连接或由工作流调用/);
  assert.match(pluginsEn, /<h1>Plugins<\/h1>/);
  assert.match(pluginsEn, /WeCom/);
  assert.match(pluginsEn, /Planned/);
  assert.match(plugins, /<details class="plugin-advanced" data-plugin-advanced>/);
  assert.doesNotMatch(plugins, /<details class="plugin-advanced" data-plugin-advanced open>/);
  assert.doesNotMatch(plugins, /<option value="local_plugin">/);
  assert.match(plugins, /item\.source_kind !== "local_plugin"/);
