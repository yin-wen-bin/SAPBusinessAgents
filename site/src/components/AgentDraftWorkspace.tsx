...
export default function AgentDraftWorkspace({ initialDraft, apiBase, locale, runPath, initialStep, onBack, onPublished }: Props) {
  ...
  const stepLabels: Record<Step, string> = {
    purpose: tr("明确用途", "Define purpose"), io: tr("设置输入与结果", "Set inputs and results"),
  ...
  return <main className={`agent-management agent-draft-workspace${assistantOpen ? " assistant-open" : ""}`}>
    ...
    <div className="draft-wizard-layout">
      ...
      <div className="draft-wizard-main">
        ...
        {step === "io" && <div className="agent-document-body"><section className="agent-panel"><div className="draft-section-heading"><div><h2>{stepLabels.io}</h2><p>{tr("编辑已有公开输入的显示信息、必填状态和默认值。字段结构、类型和复杂依赖请交给 AI 助手。", "Edit display text, required state and defaults for existing public inputs. Use the AI assistant for structure, types and complex dependencies.")}</p></div><button className="agent-secondary-action" onClick={() => openAssistant("revise", tr("请调整 Agent 的输入或输出定义。", "Please adjust the Agent input or output definition."))}>{tr("让 AI 调整", "Ask AI to adjust")}</button></div><div className="draft-input-definition-list">{Object.entries(schema.properties || {}).filter(([, property]: [string, any]) => publicInput(property)).map(([key, property]: [string, any]) => { const type = Array.isArray(property.type) ? property.type.find((item: string) => item !== "null") : property.type; const complexRequired = Boolean(schema.oneOf?.some((item: any) => item.required?.includes(key)) || Object.values(schema.dependentRequired || {}).some((items: any) => items.includes(key))); const editableDefault = !property["x-sapba-sensitive"] && !property["x-sapba-server-default"] && ["string", "number", "integer", "boolean", "array"].includes(type) && !(type === "array" && property.items?.type === "object"); const defaultValue = property.default === undefined ? "__unset__" : type === "array" ? property.default.join("\n") : String(property.default); return <fieldset className="draft-input-definition" key={key}><legend><code>{key}</code> · {type}</legend><div className="draft-basic-grid">{(["zh", "en"] as const).map((lang) => <div key={lang}><label>{lang === "zh" ? tr("中文名称", "Chinese name") : tr("英文名称", "English name")}<input disabled={locked} value={property.title?.[lang] || ""} onChange={(event) => updateInputProperty(key, (next) => { next.title = { ...(next.title || {}), [lang]: event.target.value }; })} /></label><label>{lang === "zh" ? tr("中文说明", "Chinese description") : tr("英文说明", "English description")}<textarea disabled={locked} rows={2} value={property.description?.[lang] || ""} onChange={(event) => updateInputProperty(key, (next) => { next.description = { ...(next.description || {}), [lang]: event.target.value }; })} /></label></div>)}</div><label className="draft-checkbox"><input type="checkbox" checked={(schema.required || []).includes(key)} disabled={locked || complexRequired} onChange={(event) => updateInputProperty(key, (_next, root) => { const required = new Set(root.required || []); event.target.checked ? required.add(key) : required.delete(key); root.required = [...required]; })} />{tr("根级必填", "Required at root")}</label>{complexRequired && <p>{tr("该字段参与条件必填或依赖关系，请通过 AI 助手调整。", "This field participates in conditional requirements; use the AI assistant to adjust it.")}</p>}{editableDefault && <label>{tr("默认值", "Default value")}{type === "boolean" ? <select value={defaultValue} disabled={locked} onChange={(event) => setInputDefault(key, event.target.value, property)}><option value="__unset__">{tr("未设置", "Not set")}</option><option value="true">true</option><option value="false">false</option></select> : type === "array" ? <><textarea rows={3} disabled={locked} value={defaultValue === "__unset__" ? "" : defaultValue} onChange={(event) => setInputDefault(key, event.target.value, property)} /><button type="button" className="agent-secondary-action" disabled={locked || property.default === undefined} onClick={() => setInputDefault(key, "__unset__", property)}>{tr("清除默认值", "Clear default")}</button></> : <><input type={type === "number" || type === "integer" ? "number" : "text"} disabled={locked} value={defaultValue === "__unset__" ? "" : defaultValue} onChange={(event) => setInputDefault(key, event.target.value, property)} /><button type="button" className="agent-secondary-action" disabled={locked || property.default === undefined} onClick={() => setInputDefault(key, "__unset__", property)}>{tr("清除默认值", "Clear default")}</button></>}</label>}{!editableDefault && <p>{property["x-sapba-sensitive"] ? tr("敏感字段不保存默认值。", "Sensitive fields do not store defaults.") : property["x-sapba-server-default"] ? tr("默认值由服务端提供。", "The server provides this default.") : tr("复杂默认值请通过 AI 助手调整。", "Use the AI assistant for complex defaults.")}</p>}<button type="button" className="agent-secondary-action" onClick={() => openAssistant("revise", tr(`请调整输入字段 ${key}。`, `Please adjust input field ${key}.`), { fieldPath: `/manifest/execution/inputSchema/properties/${key}` })}>{tr("让 AI 调整此字段", "Ask AI to adjust this field")}</button></fieldset>; })}</div></section><section className="agent-panel"><h2>{tr("结果结构预览", "Result structure preview")}</h2><p>{tr("这里展示定义中的结果结构，不代表真实运行结果。真实业务结果将在试运行后显示。", "This previews the defined result structure; it is not a real run result. Actual business output appears after a trial.")}</p>{Object.keys(manifest.execution?.outputSchema?.properties || {}).length ? <ul>{Object.entries(manifest.execution.outputSchema.properties).map(([key, property]: [string, any]) => <li key={key}><strong>{localText(property.title, locale) || key}</strong> <code>{Array.isArray(property.type) ? property.type.join(" | ") : property.type || "unknown"}</code></li>)}</ul> : <ul>{(manifest.outputs?.[locale] || []).map((item: string, index: number) => <li key={index}>{item}</li>)}</ul>}</section></div>}
    ...
    <AgentPublicationProgress open={publicationDialogOpen} value={publication} locale={locale} connectionError={publicationConnectionError} onClose={() => setPublicationDialogOpen(false)} onRetrySite={retrySiteRefresh} onComplete={finishPublication} />
