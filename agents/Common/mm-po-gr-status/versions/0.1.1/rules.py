...
def evaluate(inputs):
    ...
    requested_group = _text(run_input, "purchasing_group")
    requested_plant = _text(run_input, "plant")
    requested_supplier = _text(run_input, "supplier")
    requested_purchase_order = _text(run_input, "purchase_order")
    target_rows = [
        row
        for row in schedules.values()
        if _date_text(row.get("ScheduleLineDeliveryDate")) == requested_date
        and (not requested_purchase_order or _text(row, "PurchasingDocument") == requested_purchase_order)
    ]
    selected_targets = []
    for row in target_rows:
        purchase_order = _text(row, "PurchasingDocument")
        item_key = (purchase_order, _text(row, "PurchasingDocumentItem"))
        item = items.get(item_key)
        header = headers.get((purchase_order,))
        if requested_plant and (item is None or _text(item, "Plant") != requested_plant):
            continue
        header_filter_requested = bool(requested_org or requested_group or requested_supplier)
        if header_filter_requested:
            if header is None:
                continue
            if requested_org and _text(header, "PurchasingOrganization") != requested_org:
                continue
            if requested_group and _text(header, "PurchasingGroup") != requested_group:
                continue
            if requested_supplier and _text(header, "Supplier") != requested_supplier:
                continue
        elif header is None:
