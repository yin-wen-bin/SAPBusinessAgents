# 预算滚动预测助手 / Budget Rolling Forecast Assistant

## 用途与使用场景 / Purpose and scenario

在月度预算复核时，比较成本中心累计实际与全年计划，查看按月均形成的滚动预测及风险阈值。

During budget review, compare cost-center year-to-date actuals with full-year plan and inspect the monthly-average forecast and risk threshold.
...
计划证据缺失时不能据此认定预算充足。该规则是可解释的外推，不是统计预测模型，也不写回计划。

Missing plan evidence cannot establish budget sufficiency. This is transparent extrapolation, not a statistical forecast model, and it does not write plans.
...
网页 / Web: [zh](http://127.0.0.1:4321/zh/agents/CO/budget-rolling-forecast/) · [en](http://127.0.0.1:4321/en/agents/CO/budget-rolling-forecast/)

当前不能执行；完成证据与验收门禁后才可启用执行。 / Execution is blocked until evidence and acceptance gates pass.
验收模式 / Acceptance mode: `not_recorded` · 原记录日期 / Recorded date: 2026-08-20T07:45:39.722374+00:00
...
| `current_period` | 当前期间 / Current Period | 必填 / Required | `string` | {"pattern": "^[0-9]{3}$"} |
| `planning_category` | 计划类别 / Planning Category | 必填 / Required | `string` | {"minLength": 1, "maxLength": 10} |
| `risk_threshold_pct` | 风险阈值（%） / Risk Threshold Pct | 可选 / Optional | `number` | {"default": 10} |
