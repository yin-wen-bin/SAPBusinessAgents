# CO 月结分配与结算助手 / CO Month-End Allocation and Settlement Assistant

## 用途与使用场景 / Purpose and scenario

月结前检查CO期间过账、分配循环、对象状态及结算规则，定位仍需人工核实的对象。

Before closing, review CO period postings, allocation cycles, object status and settlement rules to identify objects requiring follow-up.
...
分配循环、对象状态与结算规则未取得充分证据前，不声称已分配或已结算，不执行分配或结算。

Without sufficient cycle, object-status and settlement-rule evidence, do not conclude that allocation or settlement is complete; no such transactions are executed.
...
网页 / Web: [zh](http://127.0.0.1:4321/zh/agents/CO/co-month-end-allocation-settlement/) · [en](http://127.0.0.1:4321/en/agents/CO/co-month-end-allocation-settlement/)

当前不能执行；完成证据与验收门禁后才可启用执行。 / Execution is blocked until evidence and acceptance gates pass.
验收模式 / Acceptance mode: `not_recorded` · 原记录日期 / Recorded date: 2026-08-20T08:13:43.407126+00:00
...
| `controlling_area` | 控制范围 / Controlling Area | 必填 / Required | `string` | {"minLength": 1, "maxLength": 4} |
| `period` | 会计期间 / Period | 必填 / Required | `string` | {"pattern": "^[0-9]{1,3}$"} |
| `internal_order` | 内部订单 / Internal Order | 必填 / Required | `string` | {"minLength": 1, "maxLength": 12, "pattern": "^[0-9A-Za-z_-]+$"} |
| `allocation_cycle` | 分配循环 / Allocation Cycle | 必填 / Required | `string` | {"minLength": 1, "maxLength": 10, "pattern": "^[0-9A-Za-z_-]+$"} |
