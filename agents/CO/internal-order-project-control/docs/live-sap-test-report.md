...
- SAP GET：4；证据行计数：1；耗时：25872.604 ms
- 查询源完整：`false`；业务完整：`false`
- 错误代码：run_failed
- 证据 SHA-256：`d1269fd699b266fee40d623fc9c5d19a75456def72807113e89b9d812387f4d6`

## Step status

- Embedded API：`read_order_actual`=`complete`, `read_wbs_actual`=`complete`, `read_order_plan`=`complete`, `read_wbs_plan`=`complete`
- ADT：`adt_order_master`=`failed/run_failed`, `adt_wbs_master`=`skipped/condition_false`, `adt_budget`=`skipped/condition_false`, `adt_commitment`=`skipped/condition_false`
...
- Missing evidence：budget, commitment, control_object_not_found, master, master_evidence
