# MRP 异常分析助手 / MRP Exception Analysis Assistant

## 用途与使用场景 / Purpose and scenario

按物料和MRP范围查看MRP主数据、覆盖情况与异常元素，识别短缺或重排程需要人工核实的项目。

Inspect material/MRP-area master data, coverage and exception elements to identify shortage or rescheduling follow-up.
...
说明对应当前执行定义中的读取步骤；不承诺调用未声明的MRP Skill，不运行MRP、不改计划订单。

The guide describes only declared evidence-reading steps; it does not claim an undeclared MRP Skill call, run MRP or change planned orders.
...
网页 / Web: [zh](http://127.0.0.1:4321/zh/agents/PP/mrp-exception-analysis/) · [en](http://127.0.0.1:4321/en/agents/PP/mrp-exception-analysis/)

验收模式 / Acceptance mode: `not_recorded` · 原记录日期 / Recorded date: 2026-08-24T17:16:32.109796+00:00
...
|---|---|---|---|---|
| `plant` | MRP 工厂 / MRP plant | 必填 / Required | `string` | {"minLength": 1, "maxLength": 4, "pattern": "^[0-9A-Za-z_-]+$"} |
| `mrp_area` | MRP 范围 / MRP area | 必填 / Required | `string` | {"minLength": 1, "maxLength": 10, "pattern": "^[0-9A-Za-z_-]+$"} |
| `material` | 物料 / Material | 必填 / Required | `string` | {"minLength": 1, "maxLength": 40, "pattern": "^[0-9A-Za-z_-]+$"} |
| `shortage_profile` | 短缺参数文件 / Shortage profile | 必填 / Required | `string` | {"minLength": 1, "maxLength": 20, "pattern": "^[0-9A-Za-z_-]+$"} |
| `shortage_counter` | 短缺计数器 / Shortage profile counter | 必填 / Required | `string` | {"minLength": 1, "maxLength": 3, "pattern": "^[0-9]+$"} |
...
- MRP 范围 / MRP area
- 物料 / Material
- 短缺参数文件 / Shortage profile
- 短缺计数器 / Shortage profile counter
- 分析日期 / Analysis date
- 短缺状态 / Shortage status
- 短缺数量 / Shortage quantity
- 短缺开始日期 / Shortage start date
- 短缺结束日期 / Shortage end date
- 库存覆盖天数 / Days of supply
- SAPBusinessAgents 业务处理优先级 / SAPBusinessAgents business priority
- 重排状态 / Rescheduling status
- 受影响 MRP 元素数 / Affected MRP elements
- 异常消息数 / Exception messages
- 业务证据完整性 / Business-evidence completeness
- MRP 异常明细 / MRP exception details
