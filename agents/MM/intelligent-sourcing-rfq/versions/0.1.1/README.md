# 智能寻源与 RFQ 评估助手 / Intelligent Sourcing and RFQ Evaluation Assistant

## 用途与使用场景 / Purpose and scenario

在询价评审时对照RFQ、供应商报价、报价比较与货源证据，查看固定权重形成的候选排序。

During sourcing review, compare RFQs, quotations, quotation comparisons and source evidence, then inspect fixed-weight rankings.
...
核对数量、价格、币种、交期和来源完整性后再作人工采购决定；助手不创建询价、不发邮件、不授标。

Check quantity, price, currency, delivery dates and source completeness before a purchasing decision; the Agent does not create RFQs, send mail or award business.
...
网页 / Web: [zh](http://127.0.0.1:4321/zh/agents/MM/intelligent-sourcing-rfq/) · [en](http://127.0.0.1:4321/en/agents/MM/intelligent-sourcing-rfq/)

验收模式 / Acceptance mode: `not_recorded` · 原记录日期 / Recorded date: 2026-08-20T08:14:23.179244+00:00
...
|---|---|---|---|---|
| `rfq` | 询价单 / RFQ | 必填 / Required | `string` | {"minLength": 1, "maxLength": 10, "pattern": "^[0-9A-Za-z]+$"} |
