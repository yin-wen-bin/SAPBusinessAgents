# 供应商绩效与交付风险助手 / Supplier Performance and Delivery Risk Assistant

## 用途与使用场景 / Purpose and scenario

以采购订单计划行为粒度核对净收货与日期，查看供应商OTIF、交付偏差和仍需跟进的订单。

Compare net receipts and dates at PO schedule-line grain to review supplier OTIF, delivery variance and open follow-up.
...
先确认统计窗口、收货冲销与完整性。供应商整体指标不能替代逐计划行证据；完整性不足时不强行评分。

Confirm the reporting window, receipt reversals and completeness first. Supplier aggregates do not replace schedule-line evidence; incomplete sources cannot support a forced score.
...
版本 / Version: **0.2.3** · 使用中 / Active · 验收通过 / Passed

网页 / Web: [zh](http://127.0.0.1:4321/zh/agents/MM/supplier-performance-risk/) · [en](http://127.0.0.1:4321/en/agents/MM/supplier-performance-risk/)

验收模式 / Acceptance mode: `not_recorded` · 原记录日期 / Recorded date: 2026-08-20T08:36:27.815819+00:00
证据范围 / Evidence scope: `complete`
复用 0.2.2 验收；文案复核不代表重新查询 SAP。 / Acceptance reused from 0.2.2; documentation review is not a new SAP test.
...
|---|---|---|---|---|
| `supplier` | 供应商 / Supplier | 必填 / Required | `string` | {"minLength": 1, "maxLength": 10} |
| `purchasing_organization` | 采购组织 / Purchasing organization | 必填 / Required | `string` | {"minLength": 1, "maxLength": 4, "pattern": "^[0-9A-Za-z_-]+$"} |
| `plant` | 工厂 / Plant | 可选 / Optional | `string` | {"minLength": 1, "maxLength": 4, "pattern": "^[0-9A-Za-z_-]+$"} |
| `date_from` | 开始日期 / Date From | 必填 / Required | `string` | {"format": "date"} |
| `date_to` | 结束日期 / Date To | 必填 / Required | `string` | {"format": "date"} |
