# 外购件短缺采购响应助手 / Material Shortage Procurement Response Assistant

## 用途与使用场景 / Purpose and scenario

定位外购物料MRP短缺，结合采购申请、批准状态、采购计划行与货源提出采购跟进建议。

Identify externally procured material shortages and combine requisitions, release status, PO schedules and sources into procurement follow-up.
...
关注短缺日期、数量、可跟进的申请或订单以及联系方式来源。只读建议不创建采购单，也不将未来收货视为已经到库。

Focus on shortage dates and quantities, actionable requisitions/orders and contact evidence. Advice creates no purchasing document and does not treat future receipts as stock already on hand.
...
网页 / Web: [zh](http://127.0.0.1:4321/zh/agents/MM/material-shortage-procurement-response/) · [en](http://127.0.0.1:4321/en/agents/MM/material-shortage-procurement-response/)

验收模式 / Acceptance mode: `not_recorded` · 原记录日期 / Recorded date: 2026-08-22T18:13:42.891729+00:00
...
| `mrp_area` | MRP 区域 / MRP Area | 必填 / Required | `string` | {"minLength": 1, "maxLength": 10, "pattern": "^[0-9A-Za-z_-]+$"} |
| `purchasing_organization` | 采购组织 / Purchasing organization | 必填 / Required | `string` | {"minLength": 1, "maxLength": 4, "pattern": "^[0-9A-Za-z_-]+$"} |
| `shortage_profile` | 短缺参数文件 / Shortage Profile | 必填 / Required | `string` | {"minLength": 1, "maxLength": 20, "pattern": "^[0-9A-Za-z_-]+$", "default": "SAP000000001", "x-sapba-server-default": true} |
| `shortage_counter` | 短缺定义序号 / Shortage Definition Counter | 必填 / Required | `string` | {"minLength": 1, "maxLength": 3, "pattern": "^[0-9]+$", "default": "001", "x-sapba-server-default": true} |
