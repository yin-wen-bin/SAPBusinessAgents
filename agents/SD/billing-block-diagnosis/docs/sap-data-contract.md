...
- 对象固定为 `VBUV`；字段固定为 `VBELN`、`POSNR`、`ETENR`、`TBNAM`、`FDNAM`、`FEHGR`、`STATG`。这些字段已通过目标系统实时DDIC验证；未验证字段不得加入查询。
...
- `partial`、`failed`、零行、缺行、重复/额外键或达到行数上限均保持 `inconclusive`；不得把空值或缺失证据当成完整状态。
