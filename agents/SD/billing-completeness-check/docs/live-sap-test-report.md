# 真机SAP测试报告：Billing Completeness Check
...
- API_BILLING_DOCUMENT_SRV.A_BillingDocument（5条）
- API_BILLING_DOCUMENT_SRV.A_BillingDocumentItem（5条）
- 销售订单和交货schema已验证
...
通过：覆盖超量开票、金额/币种差异、缺税码、取消与重复引用。

5条开票项目完成脱敏比较；本批未发现完整性异常。

## LLM-first结果

结果不稳定：至少一次成功选择定价元素实体，至少两次约121秒后返回 no feasible plan。
...
SAPClaw [#16](https://github.com/yin-wen-bin/SAPClaw/issues/16)。
