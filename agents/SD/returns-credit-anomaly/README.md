# 退货及贷项异常监控 / Returns and Credit Anomaly Monitor

## 用途与使用场景 / Purpose and scenario

复核退货与贷项相关记录，识别数量、退款类别或关联证据异常。

Review returns and credit-related records for quantity, refund-type or relationship anomalies.
...
退货收货与退款类型证据未齐时保持不确定，不把贷项直接视为已经退款或退货入库。

Without receipt and refund-type evidence, remain inconclusive; a credit memo does not by itself prove a refund or returned stock receipt.
...
网页 / Web: [zh](http://127.0.0.1:4321/zh/agents/SD/returns-credit-anomaly/) · [en](http://127.0.0.1:4321/en/agents/SD/returns-credit-anomaly/)

当前不能执行；完成证据与验收门禁后才可启用执行。 / Execution is blocked until evidence and acceptance gates pass.
验收模式 / Acceptance mode: `not_recorded` · 原记录日期 / Recorded date: 2026-08-20T08:39:52.862210+00:00
