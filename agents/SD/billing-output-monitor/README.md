# 发票输出监控 / Billing Output Monitor

## 用途与使用场景 / Purpose and scenario

检查发票输出和发送环节是否具有可核实的状态证据，定位待跟进发票。

Check whether invoice output/delivery has verifiable status evidence and identify follow-up invoices.
...
输出状态证据缺失时不能宣称发票已送达客户。当前可执行性以阻塞清单为准。

Missing output-status evidence cannot establish delivery to the customer. The blocker list determines current executability.
