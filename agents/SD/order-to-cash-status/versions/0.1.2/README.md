# 订单到收款状态助手 / Order-to-Cash Status

## 用途与使用场景 / Purpose and scenario

按销售订单追踪订单→交货→PGI→开票→FI清账状态，查看各阶段是否有对应凭证。

Trace a sales order through delivery, PGI, billing and FI clearing, checking document evidence at each stage.
...
这是按销售订单的端到端状态查询；按客户催收请用AR催收助手。FI清账不能证明银行实际到账。

This is sales-order-level end-to-end tracking; use AR Collection for customer-level dunning. FI clearing does not prove actual bank receipt.
...
版本 / Version: **0.1.2** · 使用中 / Active · 验收通过 / Passed

网页 / Web: [zh](http://127.0.0.1:4321/zh/agents/SD/order-to-cash-status/) · [en](http://127.0.0.1:4321/en/agents/SD/order-to-cash-status/)

验收模式 / Acceptance mode: `not_recorded` · 原记录日期 / Recorded date: 2026-09-06T15:29:30+08:00
证据范围 / Evidence scope: `complete`
复用 0.1.1 验收；文案复核不代表重新查询 SAP。 / Acceptance reused from 0.1.1; documentation review is not a new SAP test.
0.1.1仅收窄公开定义；销售订单单入口与固定Agent回归一致，原三级业务验收仍有效。
Version 0.1.1 only narrows the public definition; the sales-order-only entry matches the fixed-Agent regression and the prior three-stage business acceptance remains valid.
