# 发票争议分类 / Billing Dispute Classification

## 用途与使用场景 / Purpose and scenario

设计范围是发票拒收或争议的证据分类；当前已停用，不作为可执行的争议处理入口。

The intended scope is evidence-based classification of invoice rejection/disputes; this Agent is inactive and is not an executable dispute-management entry point.
...
缺少完整争议案例证据。不能仅凭发票或清账状态推断客户提出争议；恢复前须补齐数据与验收。

Complete dispute-case evidence is missing. Invoice or clearing status alone cannot establish a customer dispute; data and acceptance must be completed before restoration.
